...
대화를 `/종료`로 끝낸 뒤 JupyterLab의 파일 목록에서 수정할 파일을 연다.

| 파일과 항목 | 수정할 내용 |
| --- | --- |
| `prompts.json`의 `opening` | 세종의 첫 대사 |
| `prompts.json`의 `persona` | 말투, 성격, 대화 원칙 |
| `data.json`의 `content` | 세종이 참고할 역사적 사실 |
| `data.json`의 `keywords` | 해당 자료를 찾는 데 사용할 검색어 |

`data.json`에서는 `documents` 안의 원하는 자료를 선택하여 수정한다. 역사적 사실을 바꿀 때는 해당 자료의 출처를 확인한다.

`persona`의 `{intro}`와 `{evidence}`는 자기소개와 역사 자료가 들어가는 자리이므로 유지한다. JSON의 따옴표와 쉼표 형식을 지키며 수정한다.

파일을 저장한 뒤 터미널 2에서 다음 명령으로 대화 프로그램을 다시 실행한다. 터미널 1의 모델 서버는 계속 실행해 둔다.
...
새로운 자기소개부터 입력하고, 수정 전과 같은 질문을 하여 답변의 변화를 확인한다.
